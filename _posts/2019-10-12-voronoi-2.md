...
### So the list is indeed ordered by `x`
...
~~~~
region_A branch_BA- region_B branch_BA+ region_A
~~~~

While I understand how branches/`Halfedge`s are possibly ordered by the
`x` coordinate, how come that region\_A is both on the extreme left and
...
`right_of`, the algorithm is indeed capable of leveraging the ordering
by `x`, even though not in the binary search approach suggested in the
article.
